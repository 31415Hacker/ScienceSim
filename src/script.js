...
let lightSimMode = 'ray';
let lightSimModeEl = null;
let quantumControlsEl = null;
let rayControlsEl = null;
let quantumWavelengthEl = null;
let quantumWaveScaleEl = null;
let quantumApertureEl = null;
let quantumWaveletsEl = null;
let quantumPhotonsEl = null;
let quantumCoherenceEl = null;
let quantumUncertaintyEl = null;
let quantumBrightnessEl = null;
let renderQuantumEl = null;
let quantumStatusEl = null;
let lightCanvasBadgeEl = null;
let lightWatchInfoEl = null;
let quantumWavelength = 550;
let quantumWaveScale = 15;
let quantumApertureWidth = 40;
let quantumWavelets = 256;
let quantumPhotonCount = 768;
let quantumCoherence = 92;
let quantumUncertainty = 4;
let quantumBrightness = 20;
let quantumRenderSeed = 1;
let quantumRendering = false;
let quantumFrameImageData = null;
let quantumFrameDirty = true;
let quantumHasRenderedFrame = false;
let quantumSceneRevision = 0;
let quantumComputePipeline = null;
let quantumUniformBuffer = null;
let quantumShapeBuffer = null;
let quantumOutputTexture = null;
let quantumComputeBindGroup = null;
let quantumOutputTextureWidth = 0;
let quantumOutputTextureHeight = 0;
const LIGHT_SSAA = 2;
...
const LIGHT_WATCH_COPY = {
    ray: `
        <div class="meta">Yellow line: incoming laser</div>
        <div class="meta">Cyan line: refracted path through and after the shapes</div>
        <div class="meta">Warm lines: reflected paths at the boundaries</div>
        <div class="meta">Dotted normal: local surface normal where the beam hits</div>
    `,
    quantum: `
        <div class="meta">Bright regions: higher photon detection probability from constructive amplitudes</div>
        <div class="meta">Grain: shot noise from a finite number of detected photons</div>
        <div class="meta">White source: a visible-spectrum superposition rather than a single wavelength</div>
        <div class="meta">Uncertainty widens the source wavepacket before the measurement</div>
    `,
};
...
function bindQuantumDecimalInput(inputEl, labelId, stateKey, fallbackValue) {
    if (!inputEl) {
        return;
    }
    ...
    const sync = () => {
        ...
        const value = Number.isFinite(parsed) ? Math.max(0, parsed) : fallbackValue;

        inputEl.value = String(value);
        if (stateKey === 'quantumWavelets') {
            quantumWavelets = value;
        } else if (stateKey === 'quantumPhotonCount') {
            quantumPhotonCount = value;
        } else if (stateKey === 'quantumWaveScale') {
            quantumWaveScale = value;
        } else if (stateKey === 'quantumApertureWidth') {
            quantumApertureWidth = value;
        } else if (stateKey === 'quantumCoherence') {
            quantumCoherence = value;
        } else if (stateKey === 'quantumUncertainty') {
            quantumUncertainty = value;
        }
    ...
    inputEl.addEventListener('input', () => {
        ...
        if (/^-?\d*\.?\d*$/.test(trimmed) && (trimmed.endsWith('.') || trimmed === '-' || trimmed === '-.')) {
            return;
        }
        if (/^-?\d*\.?\d*$/.test(trimmed)) {
            sync();
...
function updateLightLabLabels() {
    ...
    const quantumWaveletsLabel = document.getElementById('quantumWaveletsValue');
    if (quantumWaveletsLabel) {
        quantumWaveletsLabel.textContent = String(quantumWavelets);
    }
    const quantumPhotonsLabel = document.getElementById('quantumPhotonsValue');
    if (quantumPhotonsLabel) {
        quantumPhotonsLabel.textContent = String(quantumPhotonCount);
    }
    const quantumCoherenceLabel = document.getElementById('quantumCoherenceValue');
    if (quantumCoherenceLabel) {
        quantumCoherenceLabel.textContent = String(Math.round(quantumCoherence));
    }
    const quantumUncertaintyLabel = document.getElementById('quantumUncertaintyValue');
    if (quantumUncertaintyLabel) {
        quantumUncertaintyLabel.textContent = String(Math.round(quantumUncertainty));
    }
    const quantumBrightnessLabel = document.getElementById('quantumBrightnessValue');
    if (quantumBrightnessLabel) {
        quantumBrightnessLabel.textContent = String(quantumBrightness);
    }
    const quantumWavelengthLabel = document.getElementById('quantumWavelengthValue');
    if (quantumWavelengthLabel) {
        quantumWavelengthLabel.textContent = String(Math.round(quantumWavelength));
    }
    const quantumWaveScaleLabel = document.getElementById('quantumWaveScaleValue');
    if (quantumWaveScaleLabel) {
        quantumWaveScaleLabel.textContent = String(Math.round(quantumWaveScale));
    }
    const quantumApertureLabel = document.getElementById('quantumApertureValue');
    if (quantumApertureLabel) {
        quantumApertureLabel.textContent = String(Math.round(quantumApertureWidth));
    }
    ...
    if (quantumWavelengthEl) {
        quantumWavelengthEl.value = String(quantumWavelength);
    }
    if (quantumWaveScaleEl) {
        quantumWaveScaleEl.value = String(quantumWaveScale);
    }
    if (quantumApertureEl) {
        quantumApertureEl.value = String(quantumApertureWidth);
    }
    if (quantumWaveletsEl) {
        quantumWaveletsEl.value = String(quantumWavelets);
    }
    if (quantumPhotonsEl) {
        quantumPhotonsEl.value = String(quantumPhotonCount);
    }
    if (quantumCoherenceEl) {
        quantumCoherenceEl.value = String(Math.round(quantumCoherence));
    }
    if (quantumUncertaintyEl) {
        quantumUncertaintyEl.value = String(Math.round(quantumUncertainty));
    }
    if (quantumBrightnessEl) {
        quantumBrightnessEl.value = String(quantumBrightness);
    }
    if (selectedShape) {
...
function updateSimModeVisibility() {
    const isRay = lightSimMode === 'ray';
    if (rayControlsEl) rayControlsEl.style.display = isRay ? '' : 'none';
    if (quantumControlsEl) quantumControlsEl.style.display = isRay ? 'none' : '';
}

function updateLightWatchInfo() {
    if (!lightWatchInfoEl) {
        return;
    }
    lightWatchInfoEl.innerHTML = lightSimMode === 'ray' ? LIGHT_WATCH_COPY.ray : LIGHT_WATCH_COPY.quantum;
}

function updateLightCanvasBadge() {
    if (!lightCanvasBadgeEl) {
        return;
    }

    const isQuantum = lightSimMode === 'quantum';
    let badgeText = 'Ray mode is live. Drag the source, aim handle, or any shape.';
    const isBusy = isQuantum && quantumRendering;
    const isWarning = isQuantum && !quantumRendering && quantumFrameDirty && quantumHasRenderedFrame;

    if (isQuantum) {
        if (quantumRendering) {
            badgeText = 'Rendering a quantum photon measurement on the GPU...';
        } else if (quantumFrameImageData && !quantumFrameDirty) {
            badgeText = 'Quantum measurement rendered. Adjust the setup, then measure again.';
        } else if (quantumHasRenderedFrame) {
            badgeText = 'Quantum scene changed. Click Render Updated Frame.';
        } else {
            badgeText = 'Quantum mode is frame-based. Click Render Frame to measure photon arrivals.';
        }
    }

    lightCanvasBadgeEl.textContent = badgeText;
    lightCanvasBadgeEl.classList.toggle('mode-quantum', isQuantum);
    lightCanvasBadgeEl.classList.toggle('is-warning', isWarning);
    lightCanvasBadgeEl.classList.toggle('is-busy', isBusy);
}

function updateQuantumRenderUi() {
    updateSimModeVisibility();
    updateLightWatchInfo();
    updateLightCanvasBadge();

    if (renderQuantumEl) {
        let buttonText = 'Render Frame';
        if (quantumRendering) {
            buttonText = 'Rendering...';
        } else if (quantumFrameDirty && quantumHasRenderedFrame) {
            buttonText = 'Render Updated Frame';
        } else if (!quantumFrameDirty && quantumHasRenderedFrame) {
            buttonText = 'Render Again';
        }

        renderQuantumEl.textContent = buttonText;
        renderQuantumEl.disabled = quantumRendering;
        renderQuantumEl.classList.toggle('rendering', quantumRendering);
    }
    ...
    let statusText = 'Ray mode stays live and uses the current geometric optics simulation.';
    const isQuantum = lightSimMode === 'quantum';
    const isBusy = isQuantum && quantumRendering;
    const isReady = isQuantum && !!quantumFrameImageData && !quantumFrameDirty;
    const isWarning = isQuantum && !quantumRendering && quantumFrameDirty && quantumHasRenderedFrame;

    if (isQuantum) {
        if (quantumRendering) {
            statusText = 'Rendering a white-spectrum quantum photon measurement on the GPU...';
        } else if (isReady) {
            statusText = 'Quantum measurement is up to date with the current setup.';
        } else if (isWarning) {
            statusText = 'Scene changed. Render again to update the quantum measurement.';
        } else {
            statusText = 'Quantum mode is frame-based. Adjust the setup, then render a white-spectrum probabilistic photon measurement on the GPU.';
        }
    }

    quantumStatusEl.textContent = statusText;
    quantumStatusEl.classList.toggle('is-ready', isReady);
    quantumStatusEl.classList.toggle('is-warning', isWarning);
    quantumStatusEl.classList.toggle('is-busy', isBusy);
}

function invalidateQuantumFrame() {
    quantumSceneRevision += 1;
    quantumFrameImageData = null;
    quantumFrameDirty = true;
    updateQuantumRenderUi();
}

function setQuantumFrameImage(imageData, sceneRevision) {
    if (sceneRevision !== quantumSceneRevision) {
        updateQuantumRenderUi();
        return false;
    }

    quantumFrameImageData = imageData;
    quantumFrameDirty = false;
    quantumHasRenderedFrame = true;
    updateQuantumRenderUi();
    return true;
}

async function initQuantumPipeline() {
    if (quantumComputePipeline || !device) return;
    const response = await fetch('../shaders/quantum.wgsl?v=20260409q4');
    const code = await response.text();
    const module = device.createShaderModule({ code });
    quantumUniformBuffer = device.createBuffer({
        size: 64,
        usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    quantumShapeBuffer = device.createBuffer({
        size: MAX_LIGHT_SHAPES * 8 * 4,
        usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    });
    quantumComputePipeline = device.createComputePipeline({
        layout: 'auto',
        compute: { module, entryPoint: 'main' },
    });
}

async function renderQuantumFrame() {
    if (!device || quantumRendering) return;
    quantumRendering = true;
    quantumRenderSeed = (quantumRenderSeed + 1) >>> 0;
    updateQuantumRenderUi();
    try {
        if (!quantumComputePipeline) {
            await initQuantumPipeline();
        }
        const w = lightLabState.width;
        const h = lightLabState.height;
        if (w < 1 || h < 1) return;
        const sceneRevision = quantumSceneRevision;

        if (!quantumOutputTexture || quantumOutputTextureWidth !== w || quantumOutputTextureHeight !== h) {
            if (quantumOutputTexture) quantumOutputTexture.destroy();
            quantumOutputTexture = device.createTexture({
                size: [w, h],
                format: 'rgba8unorm',
                usage: GPUTextureUsage.STORAGE_BINDING | GPUTextureUsage.COPY_SRC,
            });
            quantumOutputTextureWidth = w;
            quantumOutputTextureHeight = h;
        }

        quantumComputeBindGroup = device.createBindGroup({
            layout: quantumComputePipeline.getBindGroupLayout(0),
            entries: [
                { binding: 0, resource: quantumOutputTexture.createView() },
                { binding: 1, resource: { buffer: quantumUniformBuffer } },
                { binding: 2, resource: { buffer: quantumShapeBuffer } },
            ],
        });
        ...
        f32[0] = w;
        f32[1] = h;
        f32[2] = quantumWavelength;
        f32[3] = quantumWaveScale;
        f32[4] = lightLabState.laser.x;
        f32[5] = lightLabState.laser.y;
        const dir = getLaserDirection();
        f32[6] = dir.x;
        f32[7] = dir.y;
        f32[8] = quantumApertureWidth;
        u32[9] = Math.max(0, Math.round(quantumWavelets));
        u32[10] = lightLabState.shapes.length;
        f32[11] = quantumBrightness;
        u32[12] = Math.max(0, Math.round(quantumPhotonCount));
        u32[13] = quantumRenderSeed;
        f32[14] = quantumCoherence * 0.01;
        f32[15] = quantumUncertainty;
        device.queue.writeBuffer(quantumUniformBuffer, 0, uniformData);

        const shapeData = new Float32Array(MAX_LIGHT_SHAPES * 8);
        for (let i = 0; i < lightLabState.shapes.length && i < MAX_LIGHT_SHAPES; i++) {
            const s = lightLabState.shapes[i];
            const base = i * 8;
            shapeData[base] = s.type === 'circle' ? 0 : (s.type === 'prism' ? 2 : 1);
            shapeData[base + 1] = s.x;
            shapeData[base + 2] = s.y;
            shapeData[base + 3] = s.type === 'circle' ? s.radius : s.width;
            shapeData[base + 4] = s.type === 'circle' ? 0 : s.height;
            shapeData[base + 5] = s.rotationDeg * Math.PI / 180;
            shapeData[base + 6] = s.ior;
            shapeData[base + 7] = s.absorption;
        }
        device.queue.writeBuffer(quantumShapeBuffer, 0, shapeData);

        const encoder = device.createCommandEncoder();
        const pass = encoder.beginComputePass();
        pass.setPipeline(quantumComputePipeline);
        pass.setBindGroup(0, quantumComputeBindGroup);
        ...
        for (let row = 0; row < h; row++) {
            ...
            for (let col = 0; col < w * 4; col++) {
                imageData.data[dstOff + col] = rawData[srcOff + col];
            }
        }
        readbackBuffer.unmap();
        readbackBuffer.destroy();
        setQuantumFrameImage(imageData, sceneRevision);
    } catch (err) {
        console.error('Quantum render failed:', err);
    } finally {
...
function resetLightLab() {
    ...
    branchMode = 'both';
    quantumWavelength = 550;
    quantumWaveScale = 15;
    quantumApertureWidth = 40;
    quantumWavelets = 256;
    quantumPhotonCount = 768;
    quantumCoherence = 92;
    quantumUncertainty = 4;
    quantumBrightness = 20;
    quantumRenderSeed = 1;
    ...
    updateLightLabLabels();
    if (lightSimModeEl) lightSimModeEl.value = lightSimMode;
    invalidateQuantumFrame();
}
...
function renderLightLab() {
    ...
    if (lightSimMode === 'quantum' && quantumFrameImageData) {
        lightCtx.putImageData(quantumFrameImageData, 0, 0);
    }
...
function bindLightLabControls() {
    ...
    lightSimModeEl = document.getElementById('lightSimMode');
    rayControlsEl = document.getElementById('rayControls');
    quantumControlsEl = document.getElementById('quantumControls');
    quantumWavelengthEl = document.getElementById('quantumWavelength');
    quantumWaveScaleEl = document.getElementById('quantumWaveScale');
    quantumApertureEl = document.getElementById('quantumAperture');
    quantumWaveletsEl = document.getElementById('quantumWavelets');
    quantumPhotonsEl = document.getElementById('quantumPhotons');
    quantumCoherenceEl = document.getElementById('quantumCoherence');
    quantumUncertaintyEl = document.getElementById('quantumUncertainty');
    quantumBrightnessEl = document.getElementById('quantumBrightness');
    renderQuantumEl = document.getElementById('renderQuantum');
    quantumStatusEl = document.getElementById('quantumStatus');
    lightCanvasBadgeEl = document.getElementById('lightCanvasBadge');
    lightWatchInfoEl = document.getElementById('lightWatchInfo');

    if (lightSimModeEl) {
        lightSimModeEl.addEventListener('change', () => {
            lightSimMode = lightSimModeEl.value;
            updateQuantumRenderUi();
        });
    }
    if (quantumWavelengthEl) {
        quantumWavelengthEl.addEventListener('input', () => {
            quantumWavelength = parseInt(quantumWavelengthEl.value, 10) || 550;
            const el = document.getElementById('quantumWavelengthValue');
            if (el) el.textContent = quantumWavelength;
            invalidateQuantumFrame();
        });
    }
    if (quantumWaveScaleEl) {
        bindQuantumDecimalInput(quantumWaveScaleEl, 'quantumWaveScaleValue', 'quantumWaveScale', 15);
    }
    if (quantumApertureEl) {
        bindQuantumDecimalInput(quantumApertureEl, 'quantumApertureValue', 'quantumApertureWidth', 40);
    }
    if (quantumWaveletsEl) {
        bindQuantumDecimalInput(quantumWaveletsEl, 'quantumWaveletsValue', 'quantumWavelets', 256);
    }
    if (quantumPhotonsEl) {
        bindQuantumDecimalInput(quantumPhotonsEl, 'quantumPhotonsValue', 'quantumPhotonCount', 768);
    }
    if (quantumCoherenceEl) {
        bindQuantumDecimalInput(quantumCoherenceEl, 'quantumCoherenceValue', 'quantumCoherence', 92);
    }
    if (quantumUncertaintyEl) {
        bindQuantumDecimalInput(quantumUncertaintyEl, 'quantumUncertaintyValue', 'quantumUncertainty', 4);
    }
    if (quantumBrightnessEl) {
        bindQuantumDecimalInput(quantumBrightnessEl, 'quantumBrightnessValue', 'quantumBrightness', 20);
    }
    if (renderQuantumEl) {
        renderQuantumEl.addEventListener('click', () => {
            renderQuantumFrame().catch((err) => console.error('Quantum render failed:', err));
        });
    }
